...
def get_user_classes(tg, dc, include_temp=True):
    with get_db() as conn:
        cur = conn.cursor()
        cur.execute(
            "SELECT id, lesson_name, lesson_day, lesson_time, 'permanent' as type FROM classes WHERE telegram_id=? OR discord_id=?",
            (str(tg), str(dc))
        )
        ...
        if include_temp:
            ...
            cur.execute(
                "SELECT id, lesson_name, temp_day, temp_time, 'temp' as type FROM temp_moves WHERE (telegram_id=? OR discord_id=?) AND week_start=?",
...
def move_class_temp(lesson_name: str, new_day: str, new_time: str, tg: str, dc: str):
    with get_db() as conn:
        ...
        current_week = get_current_week_start()
        cur.execute("DELETE FROM temp_moves WHERE original_class_id=? AND week_start=?", (class_id, current_week))
        cur.execute(
            "INSERT INTO temp_moves (original_class_id, lesson_name, temp_day, temp_time, telegram_id, discord_id, week_start) VALUES (?, ?, ?, ?, ?, ?, ?)",
...
def clear_temp_moves(tg=None, dc=None):
    with get_db() as conn:
        ...
        current_week = get_current_week_start()
        if tg:
            cur.execute("DELETE FROM temp_moves WHERE telegram_id=? AND week_start=?", (str(tg), current_week))
        elif dc:
            cur.execute("DELETE FROM temp_moves WHERE discord_id=? AND week_start=?", (str(dc), current_week))
...
def get_class_by_name(lesson_name: str, tg: str = None, dc: str = None):
    ...
    with get_db() as conn:
        cur = conn.cursor()
        if tg:
            cur.execute("SELECT id, lesson_name, lesson_day, lesson_time FROM classes WHERE lesson_name=? AND telegram_id=?", (lesson_name, str(tg)))
        else:
            cur.execute("SELECT id, lesson_name, lesson_day, lesson_time FROM classes WHERE lesson_name=? AND discord_id=?", (lesson_name, str(dc)))
        row = cur.fetchone()
...
def get_all_classes():
    with get_db() as conn:
        cur = conn.cursor()
        cur.execute("SELECT lesson_name, lesson_day, lesson_time, telegram_id, discord_id FROM classes")
        return cur.fetchall()
...
def get_all_classes_by_owner(owner_tg: str = None, owner_dc: str = None):
    with get_db() as conn:
        ...
        if owner_tg and owner_tg != "none":
            cur.execute("SELECT lesson_name, lesson_day, lesson_time FROM classes WHERE telegram_id = ?", (str(owner_tg),))
        elif owner_dc and owner_dc != "none":
            cur.execute("SELECT lesson_name, lesson_day, lesson_time FROM classes WHERE discord_id = ?", (str(owner_dc),))
        else:
            return []
        return cur.fetchall()
...
def delete_class(lesson: str, tg=None, dc=None):
    with get_db() as conn:
        cur = conn.cursor()
        if tg:
            cur.execute("DELETE FROM classes WHERE lesson_name=? AND telegram_id=?", (lesson, str(tg)))
        elif dc:
            cur.execute("DELETE FROM classes WHERE lesson_name=? AND discord_id=?", (lesson, str(dc)))
...
def consume_share_code(code: str):
    with get_db() as conn:
        cur = conn.cursor()
        cur.execute("SELECT owner_tg, owner_dc, lesson_name FROM share_codes WHERE code=? AND expires_at > ?", (code.upper(), time.time()))
        row = cur.fetchone()
        if row:
